...
pub trait ColorAlteration<T :PixelValues<T>> {
    ...
    fn match_bgra_set_bgra(&mut self, b :u8, g :u8, r :u8, a : u8, new_b :u8, new_g :u8, new_r :u8, new_a :u8);

    /// If a color's BGRA values match a statement, set its BGRA to the provided values.
    /// bytes_matcher : |_:u8,_:u8,_:u8,a:u8| -> bool { a > 0}
    fn color_matcher_and_new_color<F: Fn(u8,u8,u8,u8) -> bool>(&mut self, bytes_matcher :F, v0 :u8, v1 :u8, v2 :u8, v3 :u8);

    /// If a color's BGRA values match a statement, set its BGRA to the provided values.
    /// bytes_matcher : |_:u8,_:u8,_:u8,a:u8| -> bool { a > 0}
    fn color_matcher_and_alterator<FM: Fn(u8,u8,u8,u8) -> bool, FA: Fn(&mut [u8])>(&mut self, bytes_matcher :FM, bytes_alterator :FA);
...
pub mod bytes_matchers {
    pub fn visible(_:u8,_:u8,_:u8,a:u8) -> bool { a > 0 }
    pub fn visible_not_white(b:u8,g:u8,r:u8,a:u8) -> bool { (b < 255 || g < 255 || r < 255) && a > 0 }
    pub fn fully_opaque(_:u8,_:u8,_:u8,a:u8) -> bool { a == 255 }
    pub fn not_fully_opaque(_:u8,_:u8,_:u8,a:u8) -> bool { a < 255 }
}
pub mod bytes_alterators {
    /// Sets the values of the given slice to 0
    pub fn all_to_zero(bgra: &mut [u8]) { bgra.fill(0) }
...
mod tests {
    ...
    fn test_u8_u32_convertion(){
        ...
        Screen::update_area_custom(&bytes_u8_bgra, 0, 0, bytes_u8_bgra.len() as u32/4, 1, PixelsSendMode::AlphaEnabled);
        ...
        image::save_buffer_with_format(format!("{}{}", "media/", "bytes_u8_rgba_from_u8_bgra_export.png"), &bytes_u8_rgba_from_u8_bgra, (bytes_u8_rgba_from_u8_bgra.len()/4).try_into().unwrap(), 1, image::ColorType::Rgba8, image::ImageFormat::Png).unwrap();
        ...
        Screen::update_area_custom(&bytes_u32_bgra, 0, 0, bytes_u32_bgra.len() as u32, 1,PixelsSendMode::AlphaEnabled);
        ...
        image::save_buffer_with_format(format!("{}{}", "media/", "bytes_u8_rgba_from_u32_rgba_export.png"), &bytes_u8_rgba_from_u32_rgba, (bytes_u8_rgba_from_u32_rgba.len()/4).try_into().unwrap(), 1, image::ColorType::Rgba8, image::ImageFormat::Png).unwrap();
    ...
    fn test_u8_u32_convertion_png_with_transparency(){
        ...
        image::save_buffer_with_format(format!("{}{}", "media/", "rgba_u8_export.png"), &image_u8_rgba_from_image_u8_bgra.bytes, image_u8_bgra.width.try_into().unwrap(), image_u8_bgra.height.try_into().unwrap(), image::ColorType::Rgba8, image::ImageFormat::Png).unwrap();
        

        let rgba_u32 = <u8>::u8_u32_casting(&image_u8_bgra.bytes);
        let image_u32_rgba = PixelsCollection::<u32>::create(image_u8_bgra.width, image_u8_bgra.height, rgba_u32).unwrap();
        Screen::update_area_custom(&image_u32_rgba.bytes, -200, 0, image_u8_bgra.width as u32, image_u8_bgra.height as u32, PixelsSendMode::AlphaEnabled);

        // image_u32_rgba has BGRA ordered bytes
        let mut bytes_u8_bgra_from_u32_bgra = <u32>::u8_u32_casting(&image_u32_rgba.bytes);
        Screen::update_area_custom(&bytes_u8_bgra_from_u32_bgra, -200, 0, image_u8_bgra.width as u32, image_u8_bgra.height as u32, PixelsSendMode::AlphaEnabled);
        // when exporting into .png we need to go from BGRA to RGBA, so swap the B and R values
        <u8>::switch_bytes(&mut bytes_u8_bgra_from_u32_bgra,0,2);
        image::save_buffer_with_format(format!("{}{}", "media/", "bytes_u8_rgba_from_u32_rgba_export.png"), &bytes_u8_bgra_from_u32_bgra, image_u8_bgra.width.try_into().unwrap(), image_u8_bgra.height.try_into().unwrap(), image::ColorType::Rgba8, image::ImageFormat::Png).unwrap();
